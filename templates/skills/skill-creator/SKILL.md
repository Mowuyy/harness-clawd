...
For `harness_clawd`, custom skills should live under the active workspace `skills/` directory so they can be discovered automatically at runtime (for example, `<workspace>/skills/my-skill/SKILL.md`).
...
```bash
scripts/init_skill.py my-skill --path {workdir}/skills
scripts/init_skill.py my-skill --path {workdir}/skills --resources scripts,references
scripts/init_skill.py my-skill --path {workdir}/skills --resources scripts --examples
```
...
Keep frontmatter minimal. In `harness_clawd`, `metadata` and `always` are also supported when needed, but avoid adding extra fields unless they are actually required.
